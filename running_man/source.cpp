...
int main()
{
	...
	Sprite m1(M1);   // Sprite ýòî íåêîòîðûé îáüåêò îáëàäàþùèé êîîðäèíàòàìè è ðàçìåðàìè (ìû áåðåì òåêñòóðó Sky è ïîìåùàåì åå âíóòðü ýòîãî îáüåêòà) / Sprite is some object that has coordinates and dimensions (we take the Sky texture and put it inside this object)
	Sprite m2(M2);
	Sprite m3(M3);
	Sprite m4(M4);
	Sprite m5(M5);
